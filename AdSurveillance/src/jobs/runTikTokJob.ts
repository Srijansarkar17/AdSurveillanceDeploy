...
export async function runTikTokJob(userId: string): Promise<number> {
  const startTime = Date.now();
  
  console.log(`🤖 TikTok Ads Job (STUB) for user: ${userId}`);
  ...
  try {
    ...
    await logExecution({
      script_run_id: `TIKTOK_STUB_${Date.now()}`,
      ...
      calculated_fields: [],
      critical_limitations: ['TikTok Ads crawler not implemented yet'],
      user_id: userId
    });
    ...
    console.error(`❌ TikTok job error:`, error.message);
    return 0;
  }
}
